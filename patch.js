...
content = content.replace(
    /pub struct OutputFactory;\s*impl OutputFactory \{\s*pub fn new\(\) -> Self \{\s*Self\s*\}/,
...
const sigs = [
    ["jurisdiction: &str,\n    ) -> Result<Vec<u8>> {", "jurisdiction: &str,\n        language: &str,\n    ) -> Result<Vec<u8>> {"],
    ["jurisdiction: &str,\r\n    ) -> Result<Vec<u8>> {", "jurisdiction: &str,\n        language: &str,\n    ) -> Result<Vec<u8>> {"],
    ["scope3_breakdown: &HashMap<u8, Scope3CategorySummary>,\n    ) -> Result<Vec<u8>> {", "scope3_breakdown: &HashMap<u8, Scope3CategorySummary>,\n        language: &str,\n    ) -> Result<Vec<u8>> {"],
    ["scope3_breakdown: &HashMap<u8, Scope3CategorySummary>,\r\n    ) -> Result<Vec<u8>> {", "scope3_breakdown: &HashMap<u8, Scope3CategorySummary>,\n        language: &str,\n    ) -> Result<Vec<u8>> {"],
    ["verification_result: &ChainVerificationResult,\n    ) -> Result<Vec<u8>> {", "verification_result: &ChainVerificationResult,\n        language: &str,\n    ) -> Result<Vec<u8>> {"],
    ["verification_result: &ChainVerificationResult,\r\n    ) -> Result<Vec<u8>> {", "verification_result: &ChainVerificationResult,\n        language: &str,\n    ) -> Result<Vec<u8>> {"],
    ["fn generate_quarantine_xlsx(&self, quarantine: &[QuarantineRow]) -> Result<Vec<u8>> {", "fn generate_quarantine_xlsx(&self, quarantine: &[QuarantineRow], language: &str) -> Result<Vec<u8>> {"],
    ["fn generate_ef_reference_xlsx(&self, _jurisdiction: &str) -> Result<Vec<u8>> {", "fn generate_ef_reference_xlsx(&self, _jurisdiction: &str, language: &str) -> Result<Vec<u8>> {"],
    ["fn generate_taxonomy_xlsx(&self, _ledger: &[LedgerRow]) -> Result<Vec<u8>> {", "fn generate_taxonomy_xlsx(&self, _ledger: &[LedgerRow], language: &str) -> Result<Vec<u8>> {"],
    ["fn generate_issa_5000_report_xlsx(&self, ledger: &[LedgerRow]) -> Result<Vec<u8>> {", "fn generate_issa_5000_report_xlsx(&self, ledger: &[LedgerRow], language: &str) -> Result<Vec<u8>> {"],
    ["fn generate_gap_analysis_xlsx(&self, results: &[GapResult]) -> Result<Vec<u8>> {", "fn generate_gap_analysis_xlsx(&self, results: &[GapResult], language: &str) -> Result<Vec<u8>> {"],
    ["fn generate_benchmark_report_xlsx(&self, results: &[BenchmarkResult]) -> Result<Vec<u8>> {", "fn generate_benchmark_report_xlsx(&self, results: &[BenchmarkResult], language: &str) -> Result<Vec<u8>> {"],
    ["fn generate_supply_chain_stress_test_xlsx(&self, results: &[SupplierRisk]) -> Result<Vec<u8>> {", "fn generate_supply_chain_stress_test_xlsx(&self, results: &[SupplierRisk], language: &str) -> Result<Vec<u8>> {"],
    ["fn generate_ixbrl_mapping_xlsx(&self, ledger: &[LedgerRow]) -> Result<Vec<u8>> {", "fn generate_ixbrl_mapping_xlsx(&self, ledger: &[LedgerRow], language: &str) -> Result<Vec<u8>> {"],
    ["fn generate_lksg_report_xlsx(&self, results: &[LksgComplianceRow]) -> Result<Vec<u8>> {", "fn generate_lksg_report_xlsx(&self, results: &[LksgComplianceRow], language: &str) -> Result<Vec<u8>> {"],
    ["ledger: &[LedgerRow],\n    ) -> Result<Vec<u8>> {", "ledger: &[LedgerRow],\n        language: &str,\n    ) -> Result<Vec<u8>> {"],
    ["ledger: &[LedgerRow],\r\n    ) -> Result<Vec<u8>> {", "ledger: &[LedgerRow],\n        language: &str,\n    ) -> Result<Vec<u8>> {"],
];

for (let [old_s, new_s] of sigs) {
    content = content.replace(old_s, new_s);
}

// Inject apply_legal_footer
content = content.replace(/(ws\d*)\.set_name\([^\)]+\)\?;/g, (match, p1) => {
    return `${match}\n        Self::apply_legal_footer(&mut ${p1}, language);`;
});
